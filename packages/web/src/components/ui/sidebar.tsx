...
>(
  (
    ...
    // Calculate widths based on state and variant
    const expandedWidth = 'var(--sidebar-width, 16rem)';
    const collapsedWidth = variant === 'floating' || variant === 'inset'
      ? 'calc(var(--sidebar-width-icon, 3rem) + 1rem)'
      : 'var(--sidebar-width-icon, 3rem)';
    const currentWidth = state === 'collapsed' && collapsible === 'icon'
      ? collapsedWidth
    ...
    return (
      <div
        ...
        data-side={side}
      >
        {/* Spacer div that reserves space for the fixed sidebar */}
        <div
          className={cn(
        ...
        <div
          ...
          style={{
            width: currentWidth,
            ...(collapsible === 'offcanvas' && state === 'collapsed' && {
              [side === 'left' ? 'left' : 'right']: `calc(var(--sidebar-width, 16rem) * -1)`
            })
          }}
          {...props}
        >
