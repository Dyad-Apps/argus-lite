...
interface OrganizationSSOTabProps {
  organizationId: string;
}

export function OrganizationSSOTab({ organizationId }: OrganizationSSOTabProps) {
  ...
  const fetchConnections = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiClient.get<SsoConnectionListResponse>(
        `/organizations/${organizationId}/sso-connections?pageSize=100`
      );
  ...
  }, [organizationId, fetchConnections]);

  const handleCreateConnection = async () => {
    if (!organizationId || !newConnectionType || !newConnectionName.trim()) return;
    ...
    try {
      ...
      await apiClient.post(`/organizations/${organizationId}/sso-connections`, {
    ...
    try {
      ...
      if (err?.data?.error?.message?.includes('linked users')) {
        if (confirm('This connection has linked users. Force delete will unlink all users. Continue?')) {
          await apiClient.delete(
            `/organizations/${organizationId}/sso-connections/${connectionId}?force=true`
          );
          fetchConnections();
        }
      } else {
  ...
  const handleTestConnection = async (connectionId: string) => {
    ...
    try {
      const result = await apiClient.post<TestResult>(
        `/organizations/${organizationId}/sso-connections/${connectionId}/test`
      );
  ...
  const handleToggleEnabled = async (connection: SsoConnection) => {
    ...
    try {
      await apiClient.patch(
        `/organizations/${organizationId}/sso-connections/${connection.id}`,
